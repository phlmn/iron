...
pub trait WriteBody: Send {
    ...
    fn write_body(&mut self, res: &mut hyper::Body);
}
...
impl Response {
    ...
    #[doc(hidden)]
    pub fn write_back(self, mut http_res: HttpResponse<hyper::Body>) -> HttpResponse<hyper::Body> {
        *http_res.headers_mut() = self.headers;

        // Default to a 404 if no response code was set
        *http_res.status_mut() = self.status.unwrap_or(Status::NOT_FOUND);

        let out = match self.body {
            Some(body) => write_with_body(http_res, body),
            None => {
                http_res.headers_mut().insert(headers::CONTENT_LENGTH, "0".parse().unwrap());
...
fn write_with_body(mut res: HttpResponse<hyper::Body>, mut body: Box<WriteBody>)
                   -> io::Result<()> {
    let content_type = res.headers().get(headers::CONTENT_TYPE)
                           .map_or_else(|| "text/plain".parse().unwrap(),
                                        |cx| cx.clone());
    res.headers_mut().insert(headers::CONTENT_TYPE, content_type);

    let hyper_body = hyper::body::Body::empty();

    body.write_body(&mut hyper_body);
...
impl Debug for Response {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln!(f, "HTTP/1.1 {}\n{:?}",
            self.status.unwrap_or(Status::NOT_FOUND),
            self.headers
        )
    }
}
